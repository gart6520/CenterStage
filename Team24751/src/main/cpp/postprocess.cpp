            ...
            for (int gx = 0; gx < gridnum; gx++) {
                ...
                for (int ch = 0; ch < 3; ch++) {
                    ...
                    if (obj_conf_unsigmoid >= revsigmoid_conf) {
                        ...
                        float cx = ((sigmoid(elmptr_ch[0]) * 2.0f) - 0.5f + (float) gx) *
                                   (float) strides;
                        float cy = ((sigmoid(elmptr_ch[1]) * 2.0f) - 0.5f + (float) gy) *
                                   (float) strides;
                        float w = (sigmoid(elmptr_ch[2]) * sigmoid(elmptr_ch[2])) * 4.0f *
                                  (float) anchorgrid[ch][0];
                        float h = (sigmoid(elmptr_ch[3]) * sigmoid(elmptr_ch[3])) * 4.0f *
                                  (float) anchorgrid[ch][1];
...
Java_org_firstinspires_ftc_team24751_subsystems_vision_Yolov5Processor_postprocess(JNIEnv *env,
                                                                                   jobject thiz,
                                                                                   jobjectArray input1,
                                                                                   jobjectArray input2,
                                                                                   jobjectArray input3,
    ...
    //Detector
    detector(&bbox_candidates, env, input1, input_size / 8, strides[0], anchorgrids[0],
             conf_thresh);
    detector(&bbox_candidates, env, input2, input_size / 16, strides[1], anchorgrids[1],
             conf_thresh);
    detector(&bbox_candidates, env, input3, input_size / 32, strides[2], anchorgrids[2],
             conf_thresh);
    //non-max-suppression
